...
class BGFF(nn.Module):
    def __init__(self, input_channels, output_channels,
        ...
        # make sure seg decoder
        assert  input_channels == 2*output_channels, '%s, %s'%(input_channels,output_channels)

        self.conv = conv_op(output_channels, output_channels, **conv_kwargs)
        ...
        self.seg_conv = nn.Conv3d(output_channels, 1, kernel_size=1)
        self.bdr_conv = nn.Conv3d(output_channels, 1, kernel_size=1)

        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        seg_feat, bdr_feat = torch.chunk(x, 2, dim=1)
        fusion_feat = seg_feat + bdr_feat

        out = self.conv(fusion_feat)
        if self.dropout is not None:
            out = self.dropout(out)
        return self.lrelu(self.instnorm(out))


class PyramidalDWConv3d(nn.Module):
    """ Inception depthweise convolution
    """

    def __init__(self, in_channels, square_kernel_size=7, band_kernel_size=9, branch_ratio=0.125):
        super().__init__()

        gc = int(in_channels * branch_ratio)  # channel numbers of a convolution branch
        self.dwconv_id = nn.Conv3d(in_channels - 3 * gc, in_channels - 3 * gc, 5, padding=5 // 2,
                                   groups=in_channels - 3 * gc)

        self.dwconv_dhw = nn.Conv3d(gc, gc, square_kernel_size, padding=square_kernel_size // 2, groups=gc)

        self.dwconv_d = nn.Conv3d(gc, gc, kernel_size=(band_kernel_size, 1, 1), padding=(band_kernel_size // 2, 0, 0),
                                  groups=gc)
        self.dwconv_hw = nn.Conv3d(gc, gc, kernel_size=(1, band_kernel_size, band_kernel_size),
                                   padding=(0, band_kernel_size // 2, band_kernel_size // 2),
                                   groups=gc)

        self.split_indexes = (in_channels - 3 * gc, gc, gc, gc)

    def forward(self, x):
        x_id, x_dhw, x_d, x_wh = torch.split(x, self.split_indexes, dim=1)
        return torch.cat(
            (self.dwconv_id(x_id), self.dwconv_dhw(x_dhw), self.dwconv_d(x_d), self.dwconv_hw(x_wh)),
            dim=1,
        )


class VPConvNeXtBlock(nn.Module):
    """
    ConvNeXt3DBlock.
    """

    def __init__(self, in_channels: int, out_channels: int, kernel_size=7, skip=True, exp_r=4,
        ...
        self.dwconv = PyramidalDWConv3d(in_channels)
...
class BasicLayer(nn.Module):
    def __init__(self, num_stage, num_only_conv_stage, num_pool, base_num_features, input_resolution, depth, num_heads,
                 ...
                 is_seg_decoder=False,
                 is_bdr_decoder=False,
                 ):
        ...
        self.is_bdr_decoder = is_bdr_decoder
        ...
        if self.is_encoder:
            if not num_stage >= num_only_conv_stage:
                ...
                # 5, 5
                split_size = [5, 5, 5, 5, 5]

                self.convnext_blocks = VPConvNeXtBlock(in_channels=input_features, out_channels=dim, kernel_size=5,
                                                       exp_r=2,
                ...
                self.conv_blocks = nn.Sequential(
                    *([basic_block(input_features, dim, conv_op,
                      ...
                      [basic_block(dim, dim, conv_op,
                                   ...
                                   norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs,
                                   nonlin, nonlin_kwargs) for _ in range(num_conv_per_stage - 1)]))
            elif self.is_seg_decoder:
                if input_features == dim:
                    self.conv_blocks = nn.Sequential(
                        *([basic_block(input_features, dim, conv_op,
                                       conv_kwargs,
                                       norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs,
                    ...
                    self.conv_blocks = nn.Sequential(
                        *([nn.Identity()] +
                          [BGFF(input_features, dim, conv_op,
                                conv_kwargs,
                                norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs,
                                nonlin, nonlin_kwargs)] +
                          [basic_block(dim, dim, conv_op,
    ...
    def forward(self, x, skip):
        ...
        # is seg decoder, x=seg_x, skip_bdr_x
...
# net totoal parameters: 43.01M (43006944)
# all: 43.01M (43007604)
class BGHNetV2(SegmentationNetwork):
    def __init__(self,
                 img_size,
                 base_num_features,
                 num_classes,
...
if __name__ == "__main__":
    ...
    # origin: 43.01M
    # all (bda): 43.59M
    network = BGHNetV2(img_size=patch_size,
    ...
    print('net totoal parameters: %.2fM ' % (count_parameters(network) / 1e6))

    # from fvcore.nn import FlopCountAnalysis
    # from fvcore.nn import parameter_count_table
    #
    # x = torch.zeros((1, 1, 80, 160, 160), requires_grad=False).cuda()
    # flops = FlopCountAnalysis(network, x)
    # print('FLOPS: %.2fG' % (flops.total() / 1000 ** 3))

    """
    input size: 80, 160, 160
    net totoal parameters: 37.22M
    FLOPS: 406.48G
    GPU: 10.2G
    """
    ...
    with torch.no_grad():
        print(network)
        x = torch.zeros((1, 1, 80, 160, 160)).cuda()
        seg_out = network(x)
        for current_out in seg_out:
            print('seg_out')
