...
class Convnext3DBlock(nn.Module):
    ...
    def __init__(self, in_channels:int, out_channels:int, exp_r=4, layer_scale_init_value=1e-6):
        super().__init__()
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, num_stage, num_only_conv_stage, num_pool, base_num_features, input_resolution, depth, num_heads,
        ...
        # InstanceNorm
        if self.is_encoder:
            self.conv_blocks = Convnext3DBlock(in_channels=input_features, out_channels=dim, exp_r=2)
        else:
...
class BAPHTransNext(SegmentationNetwork):
    def __init__(self,
                 img_size,
                 base_num_features,
                 num_classes,
        ...
        # conv num
        self.encoder_num_conv_per_stage = 1
        self.decoder_num_conv_per_stage = 2
...
#
#     network = BAPHTransNext(img_size=patch_size,
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        print(network)
        x = torch.zeros((2, 1, 80, 160, 160)).cuda()
        seg_out = network(x)
