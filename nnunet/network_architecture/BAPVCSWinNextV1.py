...
class BasicLayer(nn.Module):
    ...
    def __init__(self, num_stage, num_only_conv_stage, num_pool, base_num_features, input_resolution, depth, num_heads,
                 stripe_width, image_channels=1, num_conv_per_stage=2, conv_op=None,
        ...
        from nnunet.network_architecture.CSwinTrans.vcswin import VCSWin3DBlock
        from einops import rearrange
        from nnunet.network_architecture.swin_3D import SwinTransformerBlock
        from nnunet.network_architecture.CSwinTrans.cswin3D_hv import CSWinBlock3D


        # build blocks
        if num_stage >= num_only_conv_stage:
            self.convnext_blocks = Convnext3DBlock(in_channels=input_features, out_channels=dim, kernel_size=3, exp_r=2,
            ...
            if num_stage > 4:
                self.swin_blocks = nn.ModuleList([
                    VCSWin3DBlock(
                        ...
                        qkv_bias=qkv_bias,
                        qk_scale=qk_scale,
                        split_size=stripe_width[num_stage],
                        drop=drop,
    ...
    def forward(self, x, skip):
        ...
        if self.num_stage >= self.num_only_conv_stage:
            ...
            # print('transformer_num', len(self.swin_blocks))
            for trans_idx, tblk in enumerate(self.swin_blocks):
                # print('--' * 30)
                # print('%s-th trans block'%(str(trans_idx)))
                if self.use_checkpoint:
                    s = checkpoint.checkpoint(tblk, s)
                else:
                    # 获取实例的所有属性（初始化变量）
                    attributes = vars(tblk)

                    # #
                    # # 打印所有属性
                    # for attribute, value in attributes.items():
                    #     if '_' not in attribute[0]:
                    #         print(f"{attribute}: {value}")
                    #
                    #

                    # print('trans input:',s.shape)
                    s = tblk(s)
                    # print('trans output:', s.shape)
            x = x + s
...
class BAPVCSWinNextV1(SegmentationNetwork):
    def __init__(self,
                 img_size,
                 base_num_features,
                 num_classes,
        ...
        for i_layer in range(self.num_pool):  # 0,1,2,3
            layer = BasicLayer(num_stage=i_layer,
                               ...
                               image_channels=image_channels, num_conv_per_stage=self.encoder_num_conv_per_stage,
    ...
    def forward(self, x):
        ...
        for inx, layer in enumerate(self.up_layers):
            ...
            if inx > 0:
                # first add and conv and upsample
...
#
#     network = BAPVCSWinNextV1(img_size=patch_size,
...
#                           stripe_width=stripe_width,
#                           drop_path_rate=0)
#     #  stripe_width=[2*16, 3*32, 3*32]
#     # features 24, num_heads = [3, 6, 12, 24], max_num_features=24 * 13, FLOPs = 658.59252G, Params = 49.829832M
#     #
#     # features 32, num_heads =4, 8, 16, 32
#
#     print(network)
#
...
if __name__ == "__main__":
    ...
    seg_unified_model_opt = torch.optim.Adam(network.parameters())
    from lib.loss.BCEDice import BCEDiceLoss
    bce_dice_loss = BCEDiceLoss(n_classes=num_classes, do_softmax=True)

    x = torch.zeros((2, num_input_channels, patch_size[0], patch_size[1], patch_size[2])).cuda()
    y_gt = np.random.randint(low=0, high=num_classes, size=(x.shape[0], patch_size[0], patch_size[1], patch_size[2]))
    y_gt_tensor = torch.from_numpy(y_gt).type(torch.LongTensor).cuda()
    ...
    with autocast(enabled=True):
        ...
        loss_t2_sg_1 = bce_dice_loss(seg_out[0], y_gt_tensor)
