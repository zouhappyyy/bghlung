...
def create_grid_like(t, dim = 0):
    ...
    torch_mesh_grid = torch.meshgrid(
        torch.arange(f, device=device),
        torch.arange(h, device=device),
        torch.arange(w, device=device),
        )

    grid = torch.stack(torch_mesh_grid, dim = dim)
    # print('grid',grid)
    # # grid = torch.meshgrid(
    # #     torch.arange(f, device=device),
    # #     torch.arange(h, device=device),
...
x = torch.randn(1, 96, 40, 40, 40) # (batch, dimension, frames, height, width)
out = attn(x) # (1, 512, 10, 32, 32)
print(out.shape)
