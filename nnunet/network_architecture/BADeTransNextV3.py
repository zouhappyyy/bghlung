...
# from nnunet.network_architecture.swin_3D import *
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, num_stage, num_only_conv_stage, num_pool, base_num_features, input_resolution, depth, num_heads,
        ...
        # InstanceNorm
        if self.is_encoder:
            # self.conv_blocks = nn.Sequential(*([basic_block(input_features, dim, conv_op, conv_kwargs, norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs)] +
            #                                    [Convnext3DBlock(in_channels=dim, out_channels=dim, kernel_size=3, exp_r=2, layer_scale_init_value=0, skip=False) for _ in range(num_conv_per_stage - 1)]))

            # self.conv_blocks = Convnext3DBlock(in_channels=input_features, out_channels=dim, kernel_size=3, exp_r=2, layer_scale_init_value=0, skip=True)
            #
        ...
        if num_stage >= num_only_conv_stage:
            ...
            # print('dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop, attn_drop',
            ...
            self.swin_blocks = nn.ModuleList([
                DeformableAttention3D(
                    ...
                    downsample_factor=(8, 8, 8),  # downsample factor (r in paper)
                    offset_scale=(8, 8, 8),  # scale of offset, maximum offset
                    offset_kernel_size=(10, 10, 10),  # offset kernel size
                )
...
class BADeTransNextV3(SegmentationNetwork):
    def __init__(self,
                 img_size,
                 base_num_features,
                 num_classes,
...
if __name__ == "__main__":
    ...
    network = BADeTransNextV3(img_size=patch_size,
