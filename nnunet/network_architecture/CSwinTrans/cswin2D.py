...
class CSWinBlock(nn.Module):
    ...
    def __init__(self,
        ...
        :param reso:
        :param num_heads:
        :param split_size:default Our default setting [1,2,7,7] a stripe width and can be adjusted to balance the learning capacity and computation complexity
        :param mlp_ratio:
        :param qkv_bias:
        :param qk_scale:
        :param drop:
    ...
    def forward_features(self, x):
        ...
        for blk in self.stage1:
            if self.use_chk:
                ...
                x = blk(x)
        for stage_idx, (pre, blocks) in enumerate(zip([self.merge1, self.merge2, self.merge3],
                               [self.stage2, self.stage3, self.stage4])):

            x = pre(x)
            print('>>>' * 30)
            print('stage_idx', stage_idx+2)
            print('transformer_num', len(blocks))

            for trans_idx, blk in enumerate(blocks):
                print('--' * 30)
