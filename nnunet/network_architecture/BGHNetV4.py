...
class BGFF(nn.Module):
    def __init__(self, input_channels, output_channels,
        ...
        self.fusion_conv = nn.Conv3d(output_channels, output_channels, kernel_size=3, stride = 1, padding=3//2, groups=output_channels)
    ...
    def forward(self, x):
        ...
        seg_weight = self.sigmoid(self.seg_conv(fusion_feat))
        bdr_weight = self.sigmoid(self.bdr_conv(fusion_feat))
        out = torch.cat((seg_feat * seg_weight, bdr_feat * bdr_weight), dim=1)
        out = out + x
        out = self.conv(out)
        if self.dropout is not None:
...
# all: 43.01M (43007604)
class BGHNetV4(SegmentationNetwork):
    def __init__(self,
                 img_size,
                 base_num_features,
                 num_classes,
...
if __name__ == "__main__":
    ...
    # origin: 43.01M
    # all (bda): 43.59M
    network = BGHNetV4(img_size=patch_size,
