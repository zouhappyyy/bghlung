...
class UNet2DTrainer(nnUNetTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False):
        """
        modified from nnUNet
        """
        super(UNet2DTrainer, self).__init__(plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data,
                         deterministic, fp16)
    ...
    def initialize_network(self):
        ...
        from nnunet.network_architecture.UNet2D import UNet2D5
        self.network = UNet2D5(params={'in_chns': self.num_input_channels,
        ...
        net_param_str = 'Patch size: %s, Net totoal parameters: %.2fM (%d)' % (str(self.patch_size), param / 1e6, param)
    ...
    def run_training(self):
        ...
        super(UNet2DTrainer, self).run_training()
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        super(UNet2DTrainer, self).save_checkpoint(fname, save_optimizer)
        info = OrderedDict()
